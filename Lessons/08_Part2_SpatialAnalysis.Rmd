...
NCHUC8shp <- st_read('../Environmental_Data_Analytics_2022/Data/Spatial/NCHUC8.shp')
...
mapview(NCHUC8shp, zcol = "ACRES")

ggplot(data = NCHUC8shp) +
...
NCHUC8shp_upperneuse <- st_read('../Environmental_Data_Analytics_2022/Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN == "Upper Neuse")


#Create a map
mapview(NCHUC8shp_upperneuse)

ggplot(data = NCHUC8shp_upperneuse) +
  geom_sf()
...
Energy_sf <- st_read('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
   colClasses = c('HUC_12' = 'factor')) %>%
   mutate(HUC_8 = substr(Energy_sf$HUC_12,1,8)) %>%
          group_by(HUC_8) %>%
...
Shape, area, distance, direction - The things that you  can preserve even though you  use different projections 
Each country, state has its own coordinate system so that we can focus on that specific country 

High pollution in vulnerable places ~ a project idea !
