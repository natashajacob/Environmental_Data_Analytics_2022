...
library(lubridate)
NTL.phys.data <- read.csv("../Environmental_Data_Analytics_2022/Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)
...
`==` to compare
`!=`to check if variables are different after the operator
`<`
`<=`
`>`
`>=`
`&` if you want both to be true 
`|` if you want only one to be true 
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #Filtering rows that have a 0 on column depth
...
NTL.phys.data.1999.int <- filter(NTL.phys.data, year4 %in% c("1999"))

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.TuesdayLakeHW <- filter(NTL.phys.data, year4 >= 1990 & daynum <= 1999)
...
NTL.phys.temp.desc <- arrange(NTL.phys.data, desc(temperature_C))
# 1998/7/16 East Long Lake 0.5 temp: 34.1
```
### Select
...
You need dpylr package to use pipe function
