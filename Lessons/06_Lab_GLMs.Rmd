...
R squared value tells us how much variability ur trying to explain in the model vs how much variables are there in our model. How close the points are to the linear line. 

You mostly take the log of values when your values dont have a zero. SO the nrmally distributed plot is not really normally distributed. Coz the left tail doesnt go all the way down. 

In linear modles, we can also see if categorical variables in our model has an effect in our regression
## Set up
```{r, message = FALSE}
...
The null hypothesis must have the equality.Very important. 
The null and alternative hypotheses are always stated in terms of the population mean (mu). This can be greater or lesser. 
...
for the Shapiro wilk test, it is not a good thing if p is less than 0.05. p needs to be greater than 1 which means that the data follows a normal distribution
...
#Yes, our mean is lesser than 50 accoring to our very small p value obtained from the t test 
...
With the t test we can only check two levels 
...
T test is another way of doing one way anova but with just two levels 
...
OzoneByYear1 <- ggplot(EPAair,
                       aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
...
PMbyOzone <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(PMbyOzone)
...
PMbyOzonePlot <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point() +
...
lm1 <- lm(data = EPAair, PM2.5 ~ meanLat + meanLng + Month + Year + Ozone)
summary(lm1)
step(lm1)
...
lm2 <- lm(data = EPAair, PM2.5 ~ meanLat + meanLng + Month + Ozone)
summary(lm2)
