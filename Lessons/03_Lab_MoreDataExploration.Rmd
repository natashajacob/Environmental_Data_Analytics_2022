...
```{r}
High_Temperature_Farenheit <- c(45, 48, 35, 43, 52, 51, 60, 66, 59, 45)
Low_Temperature_Farenheit <- c(27, 27, 15, 26, 25, 33, 49, 48, 27, 22)
```
...
```{r}
CelciusConversion <- function(x){
  Celcius <- (x-32)*(5/9)
  return(Celcius)
}

High_Temperature_Celcius <- CelciusConversion(High_Temperature_Farenheit)
High_Temperature_Celcius

Low_Temperature_Celcius <- CelciusConversion(Low_Temperature_Farenheit)
Low_Temperature_Celcius
```

Combine your four vectors into a data frame and add informative column names.

```{r}
Weather_Report <- data.frame("HighTemp_Farenheit" = High_Temperature_Farenheit, "LowTemp_Farenheit" = Low_Temperature_Farenheit, "HighTemp_Celcius" = High_Temperature_Celcius, "LowTemp_Celcius" = Low_Temperature_Celcius) 
Weather_Report
```
...
sd(Weather_Report$HighTemp_Farenheit)
sd(Weather_Report$LowTemp_Farenheit)
sd(Weather_Report$HighTemp_Celcius)
sd(Weather_Report$LowTemp_Celcius)
```
...
%y  2-digit year
%Y  4-digit year
%a  abbreviated weekday (to print the names of the week)
%A  unabbreviated weekday
%b  abbreviated month (to print names of the month)
...
date_obj_today <- ymd(str_today) #Function uses the right abbriveation of that day in ymd format
date_obj_today
...
##IF you have just two numbers to represent year (the computer wont know if its in the 20th century ot 21st century)
